...
//voironoi heuristic
int voironoi(int my_board[][MAP_SIZE], Position myPos, Position enemyPos, int my_distance[][MAP_SIZE], int enemy_distance[][MAP_SIZE],bool max_turn){
	for (int i = 0; i < MAP_SIZE; i++) {
		for (int j = 0; j < MAP_SIZE; j++) {
...
map<unsigned long long, HASHE> hash_table;

unsigned long long table_bitstring[MAP_SIZE][MAP_SIZE][4];
...
void init_zobrish(){
	...
	for (int i = 0; i<MAP_SIZE; i++) {
		for (int j = 0; j<MAP_SIZE; j++) {
			for (int k = 1; k <= 3; k++) {
				table_bitstring[i][j][k] = llrand();
			}
		}
	}
}

unsigned long long z_hash(int my_board[][MAP_SIZE], Position myPos, Position enemyPos){
	...
	for (int i = 0; i<MAP_SIZE; i++) {
		for (int j = 0; j<MAP_SIZE; j++) {
			if (my_board[i][j] != 0) {
				...
				if (i == enemyPos.x && j == enemyPos.y){
					k = 3;
				}
				h = h ^ table_bitstring[i][j][k];
			}
		}
	}
	return h;
...
int probe_hash(unsigned long long key, int depth, int alpha, int beta){
	HASHE *phashe = &hash_table[key];
	if (phashe->key != key) {
		return valUNKNOWN;
	}
...
void record_Hash(unsigned long long key, int depth, int val, int hashf){
	HASHE *phashe = &hash_table[key];
	if (phashe->depth >= depth) {
		return;
	}
...
int isolated_recurrence_value(int my_board[][MAP_SIZE], Position pos, int remain_depth,bool & deep_enough_recursive) {
	// init longest res
	int init_longest_estimate = isolated_chamber_value(my_board, pos, fake_distance, 1, fake_distance);
	if (remain_depth < init_longest_estimate - 5) {
		deep_enough_recursive = false;
	}
	else {
		remain_depth = max(0,init_longest_estimate - 5);
	}
	longest_res =  init_longest_estimate/ 10 * 8;
	longest_dfs(my_board, pos, remain_depth, 0);
	return longest_res;
}
//end recursive calculation

//check whether myPos is going first
bool myTurnFirst;

//heuristic using recursive
int recursive_heuristic(int my_board[][MAP_SIZE], Position myPos, Position enemyPos,int depth_remain,bool& deep_enough_recursive) {
	int myPath, enemyPath;
	int mul_base=4;
	if (!myTurnFirst) {
		mul_base = 2;
	}
	int remain = depth_remain *5;
	myPath = isolated_recurrence_value(my_board,myPos,remain,deep_enough_recursive);
	enemyPath = isolated_recurrence_value(my_board, enemyPos, remain,deep_enough_recursive);
	return myPath - enemyPath;
}


//heuristic type 2
int heuristic_type2(int my_board[][MAP_SIZE], Position myPos, Position enemyPos,int depth_remain,bool& deep_enough_recursive){
	int value;
	value = recursive_heuristic(my_board,myPos,enemyPos,depth_remain,deep_enough_recursive) * 100;
	return value;
}
...
int heuristic_type_voroinoi(int my_board[][MAP_SIZE], Position myPos, Position enemyPos, int my_distance[][MAP_SIZE], int enemy_distance[][MAP_SIZE], bool max_turn){
	int value;
	value = voironoi(my_board,myPos,enemyPos,my_distance,enemy_distance,max_turn) * 100;
	return value;
}

int remembered_count[30];
...
int alpha_beta_prunning(int my_board[][MAP_SIZE], Position myPos, Position enemyPos, int depth, int alpha, int beta, int my_distance[][MAP_SIZE], int enemy_distance[][MAP_SIZE],int game_state) {
	...
	if ((float(current_time) - float(start_time)) / CLOCKS_PER_SEC > 2.5) {
		timeout = true;
		return 0;
	}
	//
	...
	int key = z_hash(my_board, myPos, enemyPos);
	value = probe_hash(key, depth, alpha, beta);
	if (value != valUNKNOWN) {
		//remembered_count[depth]++;
		return value;
	}
	...
	int fake_depth;
	bool deep_enough_recursive;
	if (isolated_value != -maxv) {
		fake_depth = depth;
		heuristic_value = heuristic(my_board, myPos, enemyPos, my_distance, enemy_distance, max_turn);
		heuristic_value += turn_priority;
		if (abs(heuristic_value)<=200 ) {
			deep_enough_recursive = true;
			heuristic_value = heuristic_type2(my_board, myPos, enemyPos, depth,deep_enough_recursive);
			heuristic_value += turn_priority;
			if (deep_enough_recursive) {
				fake_depth = 50;
			}
		}
		else {
		...
		/*if (heuristic_value<-400) {
			heuristic_value = -450;
		}*/
		//
		record_Hash(key, fake_depth, heuristic_value, hashfEXACT);
		return heuristic_value;
	}

	//if it's maximum depth
	if (depth == 0 ) {
		heuristic_value = heuristic_type_voroinoi(my_board, myPos, enemyPos, my_distance, enemy_distance, max_turn);
		heuristic_value += turn_priority;
		//
		if (heuristic_value>0) {
			heuristic_value = 50;
		...
		//
		record_Hash(key, depth, heuristic_value, hashfEXACT);
		return heuristic_value;
	}
	...
	if ((max_depth - depth) % 2 == 1) {
		...
		if (beta <= alpha) {
			record_Hash(key, depth, beta, hashfBETA);
		}
		else {
			record_Hash(key, depth, alpha, hashf);
		}
		return value;
	}
	// min player
		...
		if (beta <= alpha) {
			record_Hash(key, depth, alpha, hashfALPHA);
		}
		else {
			record_Hash(key, depth, beta, hashf);
		}
		return value;
	}
}
...
//if most of obstacle lie from line 1 to line 3
bool obstacleHorizontalLine = true;
bool obstacleVerticalLine = true;

void init_obstacle_line(int my_board[][MAP_SIZE]){
	int dem_obstacle = 0;
	for (int i = 4; i <= 6; i++) {
		for (int j = 0; j < MAP_SIZE; j++) {
			if (my_board[i][j] == 1) {
	...
	if (dem_obstacle >= 6) {
		obstacleHorizontalLine = true;
	}
	else {
		obstacleHorizontalLine = false;
	}
	//horizontal
	dem_obstacle = 0;
	...
	if (dem_obstacle >= 6) {
		obstacleVerticalLine= true;
	}
	else {
		obstacleVerticalLine = false;
	}
	//vertical line
	dem_obstacle = 0;
	for (int i = 1; i < MAP_SIZE; i++) {
...
int possibility_check(int my_board[][MAP_SIZE], Position myPos, Position enemyPos, int priority[], int my_distance[][MAP_SIZE], int enemy_distance[][MAP_SIZE],int game_state) {
	...
	count_function = 0;
	//count
	int re_index[4] = { 0, 1, 2, 3 };
	int value;
	int opt_value = -maxv-1;  //fixed bug
	int opt_i = -1;
	if (mybot_is_top_left()) {
		re_index[0] = 3; re_index[1] = 2; re_index[2] = 1; re_index[3] = 0;
		if (obstacleHorizontalLine) {
			re_index[0] = 2;
			re_index[1] = 3;
		}
	}
	else {
		re_index[0] = 0; re_index[1] = 1; re_index[2] = 2; re_index[3] = 3;
		if (obstacleVerticalLine) {
			re_index[0] = 1;
			re_index[1] = 0;
		}
	}
	for (int i = 0; i < 4; i++) {
		myPos.x = myPos.x + tx[ re_index[i] ];
		myPos.y = myPos.y + ty[ re_index[i] ];
		if (inside_table(myPos.x, myPos.y) && my_board[myPos.x][myPos.y] == 0) {
			my_board[myPos.x][myPos.y] = 1;
			value = alpha_beta_prunning(my_board, myPos, enemyPos, max_depth, -maxv, maxv, my_distance, enemy_distance, game_state) + priority[re_index[i]];
			//cout << "possible[" << re_index[i]+1 << "]" <<value<< endl;
			if (value>opt_value) {
				opt_value = value;
				opt_i = re_index[i];
			}
...
int state_3_solution(int my_board[][MAP_SIZE], Position pos){
	...
	for (int i = 0; i < 4; i++) {
		...
		if (inside_table(newPos.x, newPos.y) && (my_board[newPos.x][newPos.y] == 0)) {
			my_board[newPos.x][newPos.y] = 1;
			temp_length = isolated_recurrence_value(my_board,newPos,100,zero_meaning);
...
bool special_case_isolated_middle_cell = false;
...
void case_classify(int my_board[][MAP_SIZE], Position myPos, Position enemyPos) {
	...
	//check special case
	int res_special_case;
	myTurnFirst = true;
	if ((myPos.x == 0 && myPos.y == 0 && (enemyPos.x != 10 || enemyPos.y != 10)) || (myPos.x == 10 && myPos.y == 10 && (enemyPos.x != 0 || enemyPos.y != 0))) {
		myTurnFirst = false;
		if (my_board[5][5] == 1 || (my_board[4][5]==1 && my_board[6][5]==1) || (my_board[5][4]==1 && my_board[5][6]==1)){
			special_case_isolated_middle_cell = true;
		}
	}
	if (myPos.x == 0 && myPos.y == 0 || myPos.x == 10 && myPos.y == 10) {
		init_zobrish();
		init_obstacle_line(my_board);
	}
	memset(remembered_count,0,sizeof(remembered_count) );
	...
	//if it is special case 1, we clone enemy's move
	if (game_state!=3 && special_case_isolated_middle_cell) {
		res_special_case = 0;
		my_next_Pos.x = 5 + (5 - enemyPos.x);
		my_next_Pos.y = 5 + (5 - enemyPos.y);
		if (my_next_Pos.x < myPos.x) {
			res_special_case = 1;
		} 
		if (my_next_Pos.x > myPos.x) {
			res_special_case = 3;
		}
		if (my_next_Pos.y < myPos.y) {
			res_special_case = 2;
		}
		if (my_next_Pos.y > myPos.y) {
			res_special_case = 4;
		}
		Game::GetInstance()->AI_Move(res_special_case);
		cout << "clone =" << res_special_case<<endl;
		return;
	}
	//
	...
	cout << max_depth - 1<<" "<<res<<"||remembered: "<<endl;
	/*for (int i = 0; i < 30; i++) {
		cout << remembered_count[i] << " ";
	}*/
	cout << endl;
	clock_t current_time = clock();
	cout <<"time spent:"<< float(current_time) <<" "<< float(start_time) << endl;
	Game::GetInstance()->AI_Move(res);
	//cout << "finish count";
}
